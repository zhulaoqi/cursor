...
flowchart TB
  ...
  Load --> Backup[(备用库 dataeye_customer\n硬编码双写)]
...
| `DualStarRocksLoader` | 主库失败阻断；备用库失败仅 WARNING |
...
BI_SYNC_BACKUP_JDBC_URL = "jdbc:mysql://.../dataeye_customer"
```
...
| `BI_SYNC_DUAL_WRITE`（代码常量） | 是否双写 `dataeye_customer` |
